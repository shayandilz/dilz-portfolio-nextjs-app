            ...
            <div className="w-full py-16 flex flex-col items-center">
                ...
                {headings.length > 0 && (
                    <div className="mt-8 table-of-contents">
                        <fieldset className={'text-center border border-dark px-5 pb-2 rounded-lg'}>
                            <legend className={'font-bold px-2 text-lg'}>Table of Contents</legend>
                            <ul className={'text-start'}>
                                {headings.map((heading, index) => (
                                    <li className={'py-[6px] hover:underline transition duration-150 ease-in-out'} key={index}>
                ...
                <div className="mt-8 post-content prose max-w-4xl text-justify" dangerouslySetInnerHTML={{ __html: contentWithIds }} />
                <p className="mt-4 text-sm text-gray-600">Published on: {post.date}</p>
                {/* Render Related Posts */}
                {relatedPosts.length > 0 && (
                    <div className="mt-8 max-w-4xl pt-8 border-t-2 border-dark">
                        <h5 className={'text-center pb-8 text-2xl font-bold'}>Related Posts</h5>
...
function addHeadingIds(content) {
    ...
    return content.replace(/<h[1-6][^>]*>(.*?)<\/h[1-6]>/gi, (match) => {
        index++;
        return match.replace(/<h([1-6])([^>]*)>/i, `<h$1$2 id="heading-${index}" class="anchor">`);
    });
}
